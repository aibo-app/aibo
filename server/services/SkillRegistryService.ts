...
import type { SkillManifest, SkillsConfig, SkillInstallOption } from '../types/settings';
...
export class SkillRegistryService {
     ...
     */
    public async getAllSkills(): Promise<SkillManifest[]> {
        ...
        for (const skill of skills) {
            ...
            (skill as any).enabled = entry?.enabled ?? (skill.source === 'workspace');
        }
     ...
     */
    public async getEnabledNodeCommands(): Promise<Array<{ name: string; description: string; skillName: string }>> {
        const skills = await this.getAllSkills();
        const commands: Array<{ name: string; description: string; skillName: string }> = [];

        for (const skill of skills) {
            if (!(skill as any).enabled) continue;
            ...
            if (nodeCommands.length === 0) continue;
     ...
     */
    public async toggleSkill(skillName: string, enabled: boolean): Promise<void> {
        const settingsService = SettingsService.getInstance();
        const config = await settingsService.getJSON<SkillsConfig>('OPENCLAW_SKILLS_CONFIG') || {};
        ...
        const { OpenClawConfigService } = await import('./OpenClawConfigService');
        await OpenClawConfigService.getInstance().generateConfig();
    }

    public async getSkillsConfig(): Promise<SkillsConfig> {
        const settingsService = SettingsService.getInstance();
        return await settingsService.getJSON<SkillsConfig>('OPENCLAW_SKILLS_CONFIG') || {};
    }
     ...
     */
    public async getSkillEnv(skillId: string): Promise<Record<string, string>> {
        const config = await this.getSkillsConfig();
     ...
     */
    public async setSkillEnv(skillId: string, env: Record<string, string>): Promise<void> {
        const settingsService = SettingsService.getInstance();
        const config = await settingsService.getJSON<SkillsConfig>('OPENCLAW_SKILLS_CONFIG') || {};
     ...
     */
    private isBinInstalled(bin: string): boolean {
        try {
     ...
     */
    public async getSkillStatusForBrain(): Promise<Array<{
        ...
        const statuses: Array<{
            id: string; name: string; description: string; emoji?: string;
            ready: boolean; missingBins: string[]; missingEnv: string[];
            installOptions?: SkillInstallOption[];
        }> = [];
     ...
     */
    public async getSkillById(id: string): Promise<SkillManifest | null> {
        const skills = await this.getAllSkills();
     ...
     */
    public getSkillContent(skill: SkillManifest): string {
    ...
    private resolveWorkspaceSkillsDir(): string | null {
        if (this.coreDir) {
    ...
    private scanDir(dir: string, source: 'bundled' | 'workspace'): SkillManifest[] {
        const skills: SkillManifest[] = [];
    ...
    private parseFrontmatter(content: string): {
        ...
        requiredEnv?: string[]; requiredBins?: string[]; anyBins?: string[];
        installOptions?: SkillInstallOption[];
    } {
        ...
        for (const line of fm.split('\n')) {
            ...
            if (k === 'description') result.description = v;
        }
    ...
    private extractCommands(content: string): string[] {
        ...
        const add = (cmd: string) => {
            ...
            for (const existing of seen) {
                if (existing.startsWith(cmd + ' ') || existing.startsWith(cmd + '\t')) return;
            }
            seen.add(cmd); commands.push(cmd);
        };
        ...
        while ((m = tableRowRegex.exec(content)) !== null) {
            ...
            if (/^(Command|Variable|Endpoint|Action|Method)$/i.test(cmd)) continue;
            if (/^[A-Z][A-Z_]+$/.test(cmd)) continue;
            if (cmd.startsWith('/') || cmd.startsWith('http')) continue;
        ...
        while ((listMatch = listLineRegex.exec(content)) !== null) {
            ...
            while ((btMatch = backtickRegex.exec(line)) !== null) {
                ...
                if (!/^[a-z][\w.-]+\s+[a-z]/.test(cmd)) continue;
                // Skip install/setup commands
                if (/^(brew|apt|pnpm|npm|pip|cargo|go|bun|npx)\s/i.test(cmd)) continue;
                // Skip generic system commands
                if (/^(curl|echo|mkdir|cd|ls|cat|export|source|chmod|sudo|git|docker|make)\b/i.test(cmd)) continue;
                // Skip if contains English stopwords (prose, not a command)
                if (/\b(the|is|are|to|for|with|from|and|or|not|this|that|can|will|has|was|been|its|your)\b/i.test(cmd)) continue;
                // Skip paths, flags, file globs
                if (cmd.includes('/') || cmd.startsWith('-') || cmd.startsWith('.')) continue;
                add(cmd);
            }
        }
        ...
        while ((m = codeBlockRegex.exec(content)) !== null) {
            ...
            if (line.startsWith('#') || /^[A-Z_]+=/.test(line) || /^(if|for|while|do|then|else)\b/.test(line)) continue;
            ...
            // Must look like a CLI command: lowercase tool + space + lowercase subcommand
            if (!/^[a-z][\w-]+\s+[a-z][\w-]*/.test(cmd)) continue;
            // Skip generic system/install commands and curl API calls
            if (/^(curl|brew|apt|pnpm|npm|pip|cargo|echo|mkdir|cd|ls|cat|export|source|chmod|sudo|docker|make|tmux|sleep|git|bun|npx|jq|sed|awk|grep|find|xargs|tee|sort|uniq|wc|diff|tar|zip|ssh|scp|rsync|wget|nc|nohup)\b/i.test(cmd)) continue;
            add(cmd);
        }
        ...
        if (commands.length === 0) {
            ...
            while ((m = headerCodeRegex.exec(content)) !== null) {
                ...
                if (/^(setup|install|config|prerequisite|reference|note|example|guardrail|usage|requirement)/i.test(header)) continue;
                add(header);
            }
        }
     ...
     */
    private extractNodeCommands(content: string): string[] {
        const commands: string[] = [];
        const toolRegex = /\*\*Tool:\*\*\s*`([^`]+)`/g;
        let m: RegExpExecArray | null;
        while ((m = toolRegex.exec(content)) !== null) {
     ...
     */
    private extractCommandDescriptions(skillPath: string): Map<string, string> {
    ...
    private findIcon(skillDir: string, skillId: string): string | undefined {
